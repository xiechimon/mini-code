...
- [ ] 流式中途屏幕上是**可见的正文**（而非仅"已生成 N 字"计数器）——以忠实终端模拟器断言
- [ ] 段落随增量**原地增长**，块以空行分界定稿后最终屏幕**恰一份**、无裸 `#`/`**` 标记、无残留
- [ ] 首个流式增量覆盖「思考中」占位行，屏幕上不残留占位信息
- [ ] `delta`/`flush`/`reset` 接口不变，`Main` 事件挂接零改动即可编译
- [ ] 中断 `flush(aborted)` 仍保留 partial 并追加「⏹ 已中断」标记
- [ ] `MarkdownRenderer` 仍为纯函数缝（输入 markdown/样式/宽度→输出），未读环境、未碰时钟
- [ ] 既有 `BlockStreamingResidueTest` 的「无裸 markdown/无重复/无进度残留」断言保持绿
