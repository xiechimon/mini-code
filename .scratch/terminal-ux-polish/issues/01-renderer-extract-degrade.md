...
- [ ] 交互与管道两模式事件输出经同一渲染器，文案与现状等价（REPL 冒烟 + 现有测试不回归）
- [ ] 样式探测单测：NO_COLOR 非空 / 非 tty / TERM=dumb 三分支去色，默认 tty 有色
- [ ] 渲染器为纯函数：不读环境、不碰时钟（耗时由调用方注入）
- [ ] 渲染器输出字符串断言的单测落地（先例 MainReplTest 缝）
- [ ] mvn test 全绿，零新依赖
