...
- [ ] default stream() 委派同步方法，既有 fake 测试零改动
- [ ] 流式实现：片段逐次回调、返回完整 Message（文本累积 + 工具调用 id/名称/参数分片拼装）
- [ ] 回退：SSE 建连失败/解析异常 → 一次非流式重试 + 日志可查
- [ ] 取消：外部取消信号及时中断读取，已收部分可用
- [ ] 集成测试：fake SSE 端到端（分块+延迟）+ 慢速发送取消时序
- [ ] 真实网关冒烟通过（有 key 时），行为与 spike 笔记一致
- [ ] mvn test 全绿
